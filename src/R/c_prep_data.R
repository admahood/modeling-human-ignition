...
if (!exists("fpa_clean")) {
  if (file.exists(file.path(fpa_clean_path, "fpa_clean.gpkg"))){
    fpa_clean <- st_read(file.path(fpa_clean_path, "fpa_clean.gpkg"))
    ...
    st_write(fpa_clean,
             file.path(fpa_clean_path, "fpa_clean.gpkg"),
             driver = "GPKG")
    system(paste0("aws s3 cp ",
                  fpa_clean_path, "/fpa_clean.gpkg ",
